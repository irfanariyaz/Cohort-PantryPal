...
router.get("/findByName", async (req, res) => {
  ...
  const data = await getRecipeByName(name).then((response) => {
...
router.get("/getRecipes/:id", async (req, res) => {
  ...
  const recipes = await getRecipesById(id).then((response) => {
    res.json(response);
  });
});
...
router.get("/category/:category", async (req, res) => {
  ...
  const recipes = await getRecipesByCategory(category).then((response) => {
    res.json(response);
  });
});
...
router.get("/search/:search", async (req, res) => {
  ...
  const recipes = await getRecipesByName(search).then((response) => {
    res.json(response);
  });
});
